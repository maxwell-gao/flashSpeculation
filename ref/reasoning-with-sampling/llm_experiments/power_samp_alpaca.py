...
if __name__ == "__main__":
    ...
    for problem, data in tqdm(enumerate(dataset[start:end]), desc="Benchmark on ALPACA"):
        ...
        mcmc_power_samp_output, _, _, acceptance_ratio = mcmc_power_samp_alp(
        ...
        print(
            tokenizer.decode(
                torch.tensor([mcmc_power_samp_output], dtype=torch.long, device=device).squeeze().to("cpu"),
    ...
    df.to_csv(
        os.path.join(
            save_str,
            model
            + "_alpaca_base_power_samp_results_"
            + str(mcmc_steps)
            + "_"
            + str(temp)
            + "_"
