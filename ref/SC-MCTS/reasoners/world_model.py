...
class BlocksWorldModel(WorldModel):
    ...
    def __init__(self, base_model: LanguageModel, prompt: dict, max_steps: int = 6, batch_size=2) -> None:
        super().__init__()
    ...
    def init_state(self) -> BWState:
        ...
        """
        return BWState(
            step_idx=0, last_blocks_state="", blocks_state=utils.extract_init_state(self.example), buffered_action=""
        )
    ...
    def update_blocks(self, block_states: str, action: BWAction) -> str:
        ...
        world_output = (
            self.base_model.generate([world_update_prompt], eos_token_id="\n", hide_input=True, temperature=0)
            .text[0]
            .strip()
        )
