...
def get_intermediate_states(domain_path, instance, config_data, shuffle=False):
    ...
    change_str = (
        response.split("-----------------------")[-1].split("Plan executed successfully")[0].strip().split("\n\n")
    )
    changes = []
    for c in change_str:
...
def load_blocksworld(config_file, domain_file, data_file=None, data_list=None, return_intermediate=False):
    ...
    for cur_instance in data_list:
        ...
        cur_data["question"] = fill_template(*instance_to_text_blocksworld(problem, False, config_data)) + "\n"
...
def extract_init_state(example):
    ...
    # print(example)
    init_statement = (
        example["question"].split("[STATEMENT]\nAs initial conditions I have that, ")[1].split("My goal")[0].strip()
    )
