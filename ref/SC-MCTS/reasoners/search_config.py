...
from reasoners.world_model import BWAction, BWState
...
class BWConfig(SearchConfig):
    ...
    def fast_reward(self, state: BWState, action: BWAction, node_id) -> tuple[float, dict]:
        ...
        reward_dict = self.base_model.reward(
            inputs, [inputs + action], self.reward_model, self_eval_prompt=self_eval_prompt, node_id=node_id
        )
    ...
    def reward(
        self, state: BWState, action: BWAction, reward_dict: dict, goal_reached: tuple[bool, float] = None
    ) -> float:
