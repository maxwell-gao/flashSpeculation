...
class ExLlamaModelV2(LanguageModel):
    def __init__(
        ...
        if critic:
            ...
            self.critic_generator = ExLlamaV2DynamicGenerator(
                model=self.critic_model, cache=self.critic_cache, tokenizer=self.critic_tokenizer
            )
        ...
        **kwargs,
        ...
        if strategy == "greedy":
            ...
            with torch.inference_mode():
                ...
                decoded = self.generator.generate(
                    prompt=inputs,
                    max_new_tokens=200,
                    min_new_tokens=50,
        ...
        print(
            f"generate takes: {generate_time}s, n_tokens: {len(decoded[0])}, speed: {generate_time / len(decoded[0])}s/token"
        )
    ...
    def get_next_token_logits(
        self, prompt: Union[str, list[str]], candidates: Union[list[str], list[list[str]]]
    ) -> list[np.ndarray]:
        ...
        with torch.no_grad():
            ...
            all_logits = self.model.forward(tokens, self.cache, last_id_only=True).squeeze(1)
    ...
    def get_loglikelihood(self, prefix: str, contents: list[str], **kwargs) -> np.ndarray:
        ...
        prefix_tokens = loglikelihood_tokenizer.encode(prefix, add_bos=True, add_eos=False).squeeze(0)

        contents_tokens = loglikelihood_tokenizer.encode(contents, add_bos=True, add_eos=False)
        ...
        with torch.no_grad():
            ...
            contents_logits = loglikelihood_model.forward(
                contents_tokens, loglikelihood_cache, last_id_only=False, preprocess_only=False
            )
            ...
            for j in range(contents_length):  # 对于每个批次中的样本
                if contents_tokens[j, i] != self.tokenizer.pad_token_id:  # 如果当前token不是填充符
                    acc_loglikelihood[j] += torch.log(
                        probs[j, contents_tokens[j, i]]
                    )  # 将该token的对数概率累加到acc_loglikelihood中
    ...
    def reward(
        ...
        **kwargs,
        ...
        self.cache.current_seq_len = 0
        full_content_logits = self.model.forward(
            full_contents_ids, self.cache, last_id_only=False, preprocess_only=False
        ).to("cuda:0")
        ...
        student_logits = student_model.forward(
            full_contents_ids, self.draft_cache, last_id_only=False, preprocess_only=False
        ...
        kl_div_batchmean = (
            F.kl_div(log_softmax_target_clamp, log_softmax_draft_clamp, reduction="batchmean").cpu().numpy()
        )
        kl_div_mean = (
            F.kl_div(log_softmax_target_clamp, log_softmax_draft_clamp, reduction="none").mean(-1).sum().item()
        )
        ...
        # 从前缀tokens的长度开始遍历到最大提示长度
        for i in range(len(prefix_ids), full_contents_ids.shape[1]):
            probs = torch.softmax(full_content_logits[:, i - 1, :], dim=-1).to(
                "cuda:0"
            )  # 对第i-1位置的logits计算softmax以得到概率分布
        ...
        if self.critic:
            with torch.inference_mode():
                try:
                    ...
                    critic_output = self.critic_generator.generate(
                        prompt=build_autoj_input(prefix, continue_contents),
                        max_new_tokens=500,
                        min_new_tokens=50,
        ...
        self.get_loglikelihood(self_eval_prompt, [self_eval_prompt + "good"], critic=True)[0]
