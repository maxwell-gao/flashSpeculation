...
class MCTSAggregation(Generic[State, Action, Example], ABC):
    def __init__(self, retrieve_answer: Callable[[State], Hashable], weight_policy: str = "edge"):
    ...
    def __call__(self, tree_state: MCTSNode[State, Action, Example]) -> Optional[Hashable]:
        ...
        def visit(cur: MCTSNode[State, Action, Example]):
            ...
            if cur.is_terminal:
                ...
                if self.weight_policy == "edge":
                    ...
                    answer_dict[answer] += cur.reward / cur.depth  # 按边反深度更新答案字典
            ...
            for answer, depths in depth_list.items():
                if self.weight_policy == "edge":
                    ...
                    answer_dict[answer] += cur.reward / np.mean(depths)  # 按边反深度平均值更新答案字典
        ...
        return max(answer_dict, key=lambda answer: answer_dict[answer])  # 返回具有最大权重的答案
...
class MCTS(SearchAlgorithm, Generic[State, Action, Example]):
    def __init__(
        ...
        self.output_trace_in_each_iter = output_trace_in_each_iter  # 设置是否在每次迭代中输出轨迹
        ...
        self.simulate_choice: Callable[[list[float]], int] = default_simulate_strategies.get(
        ...
        self.uct_with_fast_reward = uct_with_fast_reward  # 设置是否使用快速奖励进行UCT选择

        self._output_iter: list[MCTSNode] = None  # 初始化输出迭代列表为空
        self._output_cum_reward = -math.inf  # 初始化输出累积奖励为负无穷大

        self.trace_in_each_iter: list[list[MCTSNode]] = None  # 初始化每次迭代的轨迹列表为空
    ...
    def iterate(self, node: MCTSNode) -> list[MCTSNode]:
        ...
        if not self._is_terminal_with_depth_limit(path[-1]):  # 检查路径的最后一个节点是否为终端状态或达到深度限制
        ...
        if self.output_strategy == "max_iter" and path[-1].is_terminal and cum_reward > self._output_cum_reward:
            self._output_cum_reward = (
                cum_reward  # 如果是最大迭代策略并且路径最后一个节点是终端且累积奖励更大，更新输出的累积奖励
            )
        ...
        if (
            self.output_strategy == "last_terminal_iter" and path[-1].is_terminal
        ):  # 如果是最后终端节点策略并且路径最后一个节点是终端
            self._output_cum_reward = cum_reward  # 更新输出的累积奖励
            self._output_iter = path  # 更新输出的路径
    ...
    def _uct(self, node: MCTSNode) -> float:
        ...
        # 计算探索项的值
        exploration_term = self.w_exp * np.sqrt(exploration_numerator / exploration_denominator)

        print(f"Q_value: {Q_value}, w_exp: {self.w_exp}, exploration_term: {exploration_term}")
    ...
    def _uct_select(self, node: MCTSNode) -> MCTSNode:
        ...
        if self.uct_with_fast_reward or all(x.state is not None for x in node.children):
            ...
            unvisited_children = filter(lambda x: x.state is None, node.children)  # 找到所有未访问的子节点
            return max(unvisited_children, key=lambda x: x.fast_reward)  # 返回快速奖励最大的未访问子节点
    ...
    def _expand(self, node: MCTSNode):
        ...
        if node.state is None:  # 如果节点的状态为空
            ...
            node.reward, node.reward_details = self.search_config.reward(
                node.parent.state, node.action, reward_dict=node.fast_reward_details, **aux
            )

            node.is_terminal = self.world_model.is_terminal(node.state)  # 更新节点的终端状态标识
        ...
        for action in actions:  # 对每个动作
            fast_reward, fast_reward_details = self.search_config.fast_reward(node.state, action, node_id=node.id)
    ...
    def _simulate(self, path: list[MCTSNode]):
        ...
        while True:
            ...
            if self._is_terminal_with_depth_limit(node) or len(node.children) == 0:  # 如果节点是终端状态或达到深度限制
                return  # 结束模拟
            fast_rewards = [child.fast_reward for child in node.children]  # 获取所有子节点的快速奖励
            node = node.children[self.simulate_choice(fast_rewards)]  # 根据模拟策略选择下一个子节点
    ...
    def _dfs_max_reward(self, path: list[MCTSNode]) -> tuple[float, list[MCTSNode]]:
        ...
        visited_children = [x for x in cur.children if x.state is not None]  # 获取所有已访问的子节点

        if len(visited_children) == 0:  # 如果没有已访问的子节点
            return -math.inf, path  # 返回负无穷大和当前路径

        return max(
            (self._dfs_max_reward(path + [child]) for child in visited_children), key=lambda x: x[0]
        )  # 递归查找最大奖励路径
    ...
    def search(self):
        ...
        self.root = MCTSNode(
            state=self.world_model.init_state(), action=None, parent=None, calc_q=self.calc_q
        )  # 创建根节点
        ...
        for i in trange(self.n_iters, disable=True, desc="MCTS iteration"):
            ...
            if self.output_trace_in_each_iter:
                self.trace_in_each_iter.append(deepcopy(path))  # 如果需要输出轨迹，深拷贝路径并存储
        ...
        if self.output_strategy == "follow_max":  # 如果输出策略是跟随最大
            ...
            while True:
                ...
                visited_children = [x for x in cur.children if x.state is not None]  # 获取已访问的子节点

                if len(visited_children) == 0:  # 如果没有已访问的子节点
                    break

                cur = max(visited_children, key=lambda x: x.reward)  # 选择奖励最高的子节点
        ...
        **kwargs,
        ...
        if self.aggregator is not None:  # 如果存在聚合器
            result = MCTSResult(
                ...
                aggregated_result=self.aggregator(result.tree_state),  # 使用聚合器对结果进行聚合
            )
