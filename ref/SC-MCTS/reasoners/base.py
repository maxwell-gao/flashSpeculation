...
from typing import Generic, NamedTuple, Optional, Protocol, Tuple, TypeVar, Union, runtime_checkable
...
class LanguageModel(ABC):
    ...
    @abstractmethod
    def reward(self, prefix: str, contents: list[str], reward_model, **kwargs) -> np.ndarray: ...
...
class SearchConfig(ABC, Generic[State, Action, Example]):
    ...
    @abstractmethod
    def reward(self, state, action, reward_model, **kwargs) -> tuple[float, dict]: ...
...
class Evaluator:
    ...
    def evaluate(
        self, reasoner, shuffle_prompt=True, num_shot=4, start_idx=0, end_idx=0, log_dir=None, pickle_path=None
    ):
        ...
        for i, example in enumerate(
            tqdm(self.dataset, total=len(self.dataset), initial=start_idx, desc=self._dataset_name, disable=False)
        ):
            # try:
            print(f"\ndata idx: {i}")
            n_shot_prompt = self.sample_prompt(shuffle_prompt=shuffle_prompt, num_shot=num_shot)
    ...
    def evaluate_sample_chain(
        self, reasoner, shuffle_prompt=True, num_shot=4, resume=0, num_sample_chain=10, log_dir=None
    ):
        ...
        for i, example in enumerate(
            ...
            prompt = self.sample_prompt(shuffle_prompt=shuffle_prompt, num_shot=num_shot)
            output_list = []
            save_list = []
