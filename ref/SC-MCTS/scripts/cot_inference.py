        ...
        if self.model_type == "completion":
            output = (
                self.base_model.generate(
                    [inputs], hide_input=True, do_sample=True, temperature=self.temperature, eos_token_id=[13, "<\n>"]
                )
                .text[0]
                .strip()
            )
            ...
            output = (
                self.base_model.generate([inputs], hide_input=True, do_sample=True, temperature=self.temperature)
                .text[0]
                .replace("[PLAN END]", "")
                .strip()
            )
...
def main(base_model, data_path, prompt_path, start_idx=0, end_idx=0, temperature=0, log_dir=None, disable_log=False):
    ...
    reasoner = CoTReasoner(base_model, temperature=temperature, model_type="completion")
    ...
    accuracy = evaluator.evaluate(
        reasoner, shuffle_prompt=True, num_shot=4, start_idx=start_idx, end_idx=end_idx, log_dir=log_dir
    )
